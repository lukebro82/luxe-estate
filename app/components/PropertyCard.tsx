  ...
  if (variant === "featured") {
    return (
      <Link
        ...
        className={`group relative rounded-xl overflow-hidden shadow-soft bg-white cursor-pointer block ${className}`}
      >
        ...
        <div className="p-6 relative">
          ...
          <div className="flex items-center gap-6 mt-6 pt-6 border-t border-nordic-dark/5 ">
            ...
            <div className="flex items-center gap-2 text-nordic-muted text-sm">
              ...
              {property.size.toLocaleString("en-US")} m²
            </div>
          </div>
        </div>
      </Link>
    ...
    >
      <div className="relative aspect-4/3 overflow-hidden">
        <img
          ...
          src={property.images[0]}
        />
        <button className="absolute top-3 right-3 w-9 h-9 rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center text-nordic-dark hover:bg-mosque hover:text-white transition-all shadow-sm">
          <span className="material-icons text-lg leading-none">
            favorite_border
          </span>
        </button>
        <div
          className={`absolute bottom-3 left-3 ${property.type === "rent" ? "bg-mosque/90" : "bg-nordic-dark/90"} text-white text-xs font-bold px-2 py-1 rounded`}
        >
